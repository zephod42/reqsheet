...
The disposable test-data reset is CLI-only and uses the protected migration database identity. It refuses database names outside the explicit Reqsheet test allowlist, requires migration `0011`, derives tenant-owned deletion order from live foreign-key metadata, preserves `schema_migrations`, and requires an exact operator confirmation token. It does not expose an HTTP reset route or disable foreign-key enforcement.

Ordinary staff password reset is administrator-controlled, tenant-scoped, CSRF-protected, and invalidates the account password plus its authenticated session version. Administrator accounts are deliberately excluded from this reset action and retain the existing recovery boundary. The pilot deliberately defers email verification, MFA, brute-force/rate limiting, persistent-login policy, organisation ownership transfer, and broader abuse controls. The awaiting-first-login flow retains an account-claiming risk: knowledge of a new staff member’s initials can allow premature account claiming; it is not verified staff identity and requires review before wider public deployment.
