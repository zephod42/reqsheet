...
The initial application-domain schema now covers organisations, staff identities, organisation settings and rooms, effective-dated timetable configuration, recurring lessons, dated lesson occurrences, requisitions, and technician room preferences. Application services provide the validated path for timetable versions, slots, and recurring lessons; a bounded, explicit-date occurrence-generation service validates timetable spans and conflicts before inserting historical occurrences. The admin timetable editor updates an existing template ID in place when requested, preserving dated history and activation state. Teacher and technician views read dated occurrences and requisitions without creating parallel lesson data. Pilot landing, signup, setup settings, login, sessions, and role/admin checks provide the minimum browser access path.
...
Lesson duplication is a first-class teacher workflow. For lessons visible in the selected week, drag-and-drop performs a copy, never a move; an occupied target asks for simple overwrite confirmation. Drag/drop is never the only mechanism: the pop-out exposes a visible `Duplicate lesson...` fallback action for keyboard and assistive-technology use. Desktop dragging begins after primary-button movement; touch/mobile dragging begins after a long press so ordinary clicks, taps, and scrolling retain their existing meanings. Empty targets copy immediately and occupied targets ask for overwrite confirmation. Copying to another/future week, automatic week navigation, clipboard workflows, contextual menus, and undo remain deferred.
...
Organisation identity is persistent and domain-independent: each organisation has a validated, unique `tenant_slug`. The shared application uses the deployment-configured `REQSHEET_BASE_HOSTS` allowlist; `REQSHEET_CANONICAL_HOST` is the preferred public address, while `<tenant_slug>.<base-host>` under any permitted base resolves the organisation context. An unknown tenant or host produces an application-level 404, and an authenticated session whose organisation does not match the resolved host is rejected. Signup creates the organisation and Admin on a permitted generic host, then uses the one-time, short-lived tenant-bound onboarding handoff described above to move the browser to the canonical tenant hostname and establish the normal tenant-scoped session.

The permitted base hosts and canonical public host are configurable and are not hard-coded into tenant logic. The same tenant slug may be served under the production and staging domains without creating another organisation. The current DuckDNS/Pumba arrangement remains available for staging. DNS, web-server acceptance, and TLS coverage are deployment concerns. Once tenant context is resolved, the school landing/login page visibly shows the school name near or below the Reqsheet branding.
...
- Every authenticated user has a read-only My Account identity page for their own initials/code, all assigned roles, and organisation-level School Account Status. Users may change only their own password; identity details remain administrator-managed.
