...
Migration `0014` adds the organisation-scoped date display preference with a `DD/MM/YYYY` default; existing organisations continue working without reconfiguration after it is applied.

The repository currently contains migrations `0001` through `0016`. The command applies SQL files from `database/migrations/` in numeric version order and records applied versions in `schema_migrations`. It is safe to rerun after a successful migration; already-recorded versions are skipped. Migration versions and names must be unique. Restricted Codex work may author and test migration files, but applying them to a protected database is an administrator operation using the protected migration identity. Repository inspection alone cannot prove which migrations are applied to a live database; that is an intentional security boundary, not an implementation gap. Migration `0012` removes staff and organisation account-email columns, adds organisation recovery-key digest/generation/acknowledgement metadata, bounded recovery-flow/rate-limit tables, and nullable organisation subscription state/date fields. Migration `0013` removes legacy staff display-name storage, extends recovery flows for safe new-administrator creation, and adds dated-occurrence preparation state. Migration `0014` adds the organisation-scoped date display preference with a `DD/MM/YYYY` default. Migration `0015` adds nullable room archive timestamps without changing room IDs or historical timetable relationships. Migration `0016` adds server-side, tenant-scoped persistent-login credentials for the optional 30-day Remember Me cookie; it stores only hashed verifiers and preserves existing users, sessions, organisations, timetable data, dated occurrences, and requisitions. Because MySQL commits DDL statements independently, `0013` uses information-schema guards so it can safely finish after an interrupted attempt as well as apply to a clean `0012` schema. It preserves the cascading recovery-flow user foreign key; the mutually exclusive existing-user/new-initials target invariant is validated by the recovery store because MySQL prohibits a CHECK on a column involved in cascading referential actions. These migrations do not seed recovery keys, subscription dates, or remembered-login credentials.
...
Pilot sessions use PHP sessions with regenerated IDs, HttpOnly/Lax cookies, Secure cookies when HTTPS is detected, and a versioned payload. Incompatible or revoked sessions are cleared and return to the tenant login; a database/schema failure returns a controlled 503 instead of being treated as a bad login. Password changes, administrator resets, and administrator recovery revoke the affected account's existing sessions. Optional school-hosted Remember Me uses a separate Secure, HttpOnly, SameSite=Lax cookie backed by migration `0016`; its server-side credential has a fixed maximum lifetime of 30 days and is revoked by logout or account `auth_version` changes. Dynamic responses are non-storable, while CSS assets are versioned from the file modification time. When deploying this schema-dependent release, the administrator must apply migrations before serving the new code: `cd /var/www/reqsheet`, load the protected migration environment by the established administrator procedure, and run `php bin/migrate.php`. Migration `0012` is required by the current account/recovery code and has not been verified as applied on Pumba. A schema-dependent deployment may clear incompatible old sessions, but ordinary CSS/UI changes do not invalidate every user.

The public pilot signup route is `/signup`; on the generic base host it creates the first organisation and Admin account without email, issues a one-time short-lived tenant-bound onboarding handoff, and moves the browser to the tenant hostname. The protected first-database `/setup` route uses the same handoff when invoked through the configured application. The tenant consumes the handoff, establishes the tenant-scoped session, generates the organisation's first recovery key, and redirects the Admin to the one-time recovery-key presentation. The Admin must acknowledge safe storage before unrestricted access and Settings. An interrupted presentation can be replaced after password authentication. Required settings are school name, working days, first working day, and periods per day. Rooms are created through the timetable builder, and optional timings can be configured later from Settings. An Admin with incomplete settings is sent directly to `/settings`. A non-Admin sees the blocking message `Something's missing...` followed by `Settings need to be configured. Contact your admin.` rather than an empty operational screen.

Existing organisations are deliberately left with `recovery_key_digest = NULL` after migration. An accessible administrator signs in normally, opens Settings → Manage Organisation Recovery Key, confirms with the current password, saves the displayed key, and acknowledges it. Do not attempt to populate a digest manually: doing so would create an unusable key whose original was never delivered. Schools with no accessible administrator and no previously established key have no unauthenticated recovery route.

School short codes accepted for creation are exactly 3–12 lowercase ASCII letters or digits; `www` is reserved case-insensitively. Existing historical slugs containing hyphens or outside the new creation length remain routable and are not rewritten. There is currently no short-code editing UI.

My Account reads nullable `subscription_state` (`free_trial` or `paid`) and `subscription_until` from the organisation. Both fields must be set together by a future authorised entitlement/payment process. No migration or signup side effect activates a trial or paid status. When absent, every user sees `School Account Status: Not configured`. Countdown values are derived at render time from UTC calendar-date boundaries; payment collection, plan policy, trial duration, renewal automation, invoicing, and provider integration remain undecided.

## Tenant host configuration

School URLs use `<tenant_slug>.<allowed-base-domain>`. Both configured root domains remain generic public entry and sign-up routes, while `REQSHEET_CANONICAL_HOST` controls new signup/setup examples and onboarding destinations. The persistent tenant slug remains domain-independent. The shared application resolves either allowed domain to the same isolated organisation; unknown tenants and unknown hosts return an application-level 404, and an authenticated context for another organisation is rejected. Signup/setup stores a validated unique tenant slug; no editing UI currently exists, and the application does not perform registrar availability checks.
