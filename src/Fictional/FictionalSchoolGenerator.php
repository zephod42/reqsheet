...
final class FictionalSchoolGenerator
{
    ...
    private const MANIFEST_DIRECTORY = '/var/lib/reqsheet/fictional-schools';
    private const MANIFEST_FILE = self::MANIFEST_DIRECTORY . '/manifest.json';
    ...
    public static function preflight(array $environment): array
    {
        ...
        if (!is_dir(self::MANIFEST_DIRECTORY) || !is_readable(self::MANIFEST_DIRECTORY) || !is_executable(self::MANIFEST_DIRECTORY) || ((fileperms(self::MANIFEST_DIRECTORY) & 0o77) !== 0)) throw new RuntimeException('Refusing fictional generation: protected generator manifest directory is missing, inaccessible, or not owner-only.');
        if (!is_file(self::MANIFEST_FILE) || !is_readable(self::MANIFEST_FILE) || ((fileperms(self::MANIFEST_FILE) & 0o77) !== 0)) throw new RuntimeException('Refusing fictional generation: protected generator manifest is unavailable, inaccessible, or not owner-only.');
    ...
    { $start=new DateTimeImmutable('2026-08-31');$end=new DateTimeImmutable('2026-10-23');$o=$pdo->prepare('INSERT INTO lesson_occurrences (organisation_id,recurring_lesson_id,timetable_version_id,lesson_date,snapshot_teacher_user_id,snapshot_class_code,snapshot_room_code,snapshot_start_slot_id,snapshot_duration_periods) VALUES (:org,:lesson,:version,:date,:teacher,:class,:room,:slot,:duration)');$r=$pdo->prepare('INSERT INTO requisitions (lesson_occurrence_id,state,requirements_text,planning_notes,risk_assessment_text) VALUES (:id,:state,:req,:outline,:risk)');foreach($lessons as $lesson){$date=$start;while($date<=$end){if((int)$date->format('N')===$lesson['day']){$o->execute(['org'=>$org,'lesson'=>$lesson['id'],'version'=>$version['id'],'date'=>$date->format('Y-m-d'),'teacher'=>$lesson['teacher'],'class'=>$lesson['class'],'room'=>$lesson['room'],'slot'=>$lesson['slot'],'duration'=>$lesson['duration']]);$id=(int)$pdo->lastInsertId();$content=$this->lessonContent($lesson['class'],$id%7);$r->execute(['id'=>$id,'state'=>$content['state'],'req'=>$content['requisition'],'outline'=>$content['outline'],'risk'=>$content['risk']]);}$date=$date->add(new DateInterval('P1D'));}}}
    /** @return array{state:string,requisition:string,outline:string,risk:string} */
    private function lessonContent(string $class, int $variant): array
    { $private=str_starts_with($class,'L4')||str_starts_with($class,'U4')||str_starts_with($class,'L5')||str_starts_with($class,'M5')||str_starts_with($class,'U5')||str_starts_with($class,'L6')||str_starts_with($class,'U6');$outlines=$private?['L4 Chemical Reactions — Practical 2','U5 Rates of Reaction — Investigation 1','L6 Chemical Analysis — Practical 3','U6 Biology — Microscopy Review']:['Y8 Chemical Reactions — Lesson 1','7B1.1 — Cells and Microscopy','CP10a — Circuit Investigation','Y10 Forces — Practical 2'];$requisitions=['Nothing required','CP10a: 4 power supplies, leads, lamps, switches and crocodile clips; issue one set per pair.','7B1.1: 2 microscopes, prepared onion slides, iodine, coverslips and droppers.','L4 practical: 6 conical flasks, thermometers, measuring cylinders and labelled waste beakers.','Set out 12 test tubes in racks, three measuring cylinders, stopwatches and thermometers. Prepare the dilute solutions before the lesson, label the benches A–D, and leave a dry tray for used glassware. Check that the thermometers and balances are returned to the prep room after the final class.','3 N m trolleys, runways, metre rulers, blocks and masking tape; check the runway is clear before the class.','No equipment required for this lesson; students use their exercise books for the retrieval task.'];$index=$variant%count($requisitions);$nothing=$index===0||$index===6;return ['state'=>$nothing?'nothing_required':'requirements_entered','requisition'=>$requisitions[$index],'outline'=>$nothing?'Retrieval practice and a short practical demonstration.':$outlines[$variant%count($outlines)],'risk'=>$nothing?'':'Fictional department practical: teacher demonstrates the method, students wear eye protection where equipment is in use, and benches are left clear and dry.']; }
