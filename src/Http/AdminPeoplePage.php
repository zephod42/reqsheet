...
final class AdminPeoplePage
{
    ...
    private function render(array $people, ?string $message, bool $error): string
    {
        ...
        $body = '<section class="content-wide"><div class="page-header"><div><p class="eyebrow">Admin</p><h1>People</h1></div><a class="button secondary" href="/admin/timetable">Timetable</a></div>' . $notice . '<p>People belong only to this organisation. Staff codes must use three capital letters. Roles are cumulative and can be changed by an administrator.</p><div class="people-table-toolbar"><button type="button" data-open-dialog="person-add">Add Person</button></div><div class="people-table-wrap"><table class="people-table"><caption class="visually-hidden">People in this organisation</caption><thead><tr><th scope="col">Initials</th><th scope="col">Roles / permissions</th><th scope="col"><span class="visually-hidden">Actions</span></th></tr></thead><tbody>' . $rows . '</tbody></table></div>' . $dialogs . $addDialog . '</section><script>' . $this->script() . '</script>';
        return $body;
    }
