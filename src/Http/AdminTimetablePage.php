...
final class AdminTimetablePage
{
        ...
        if ($method === 'POST') {
            try {
                ...
                if (in_array($action, ['archive_room', 'restore_room', 'delete_room'], true)) {
                    ...
                    $id = match ($type) {
                        'teacher' => $resource->createTeacher($this->organisationId, (string) ($input['code'] ?? '')),
    ...
    private function resourceToolbar(array $versions, ?int $version, string $view, int $resource, array $users, array $rooms, array $classes): string
    {
        ...
        foreach ($users as $row) if ($row['is_active'] && (!array_key_exists('is_teacher', $row) || ($row['is_teacher'] ?? false) || ($row['operational_role'] ?? '') === 'teacher')) $html .= '<option value="' . $row['id'] . '"' . ($view === 'teacher' && $resource === (int) $row['id'] ? ' selected' : '') . '>' . $this->e((string) ($row['staff_identifier'] ?? '')) . '</option>';
        $html .= '</select></label><label>Room <select name="room" onchange="location.href=this.value===\'__new__\'?\'/admin/timetable?version=' . (int) $version . '&view=room&create=room\':\'/admin/timetable?version=' . (int) $version . '&view=room&resource=\'+this.value"><option value="0">Select room</option>';
    ...
    private function resourceLessonEditor(ResourceTimetableStore $store, TimetableVersion $version, string $view, int $resource, int $edit, array $query, array $users, array $rooms, array $classes): string
    {
        ...
        $html .= '<a class="close" href="' . $cancel . '">Cancel</a><p class="eyebrow">Version ' . $this->e($version->label ?: 'Untitled timetable') . '</p><h2 id="assignment-editor-title">' . ($lesson ? 'Edit Lesson' : 'Add Lesson') . '</h2>';
        $teacherName = $view === 'teacher' ? $fixedLabel : $this->resourceName('teacher', $teacherValue, $users, $rooms, $classes);
        $teacherCode = $view === 'teacher' ? $this->teacherCode($fixedValue, $users) : $this->teacherCode($teacherValue, $users);
        ...
        $html .= '<div class="form-actions"><button>' . ($lesson ? 'Save Changes' : 'Add Lesson') . '</button><a class="button secondary" href="' . $cancel . '">Cancel</a></div></form>';
    ...
    /** @param list<array{id:int,staff_identifier:?string,is_active:bool}> $users */
    ...
    /** @param list<array{id:int,staff_identifier:?string,is_active:bool}> $rows */
    private function selectFromRows(string $name, int $selected, array $rows): string
    {
        $options = [];
        foreach ($rows as $row) $options[$row['id']] = (string) ($row['staff_identifier'] ?? '');
