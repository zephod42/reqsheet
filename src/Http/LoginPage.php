...
final class LoginPage
{
    public function form(?string $message = null, string $login = ''): string
    {
        ...
        return $this->layout('<h1>Log in</h1>' . $notice . self::fields($login));
    }
    ...
    public static function fields(string $login = ''): string { return '<form method="post" action="/login"><label>Name/login<input name="login" value="' . htmlspecialchars($login, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '" required autofocus></label><label>Password<input type="password" name="password" required></label><button>Log in</button></form>'; }
