...
final class TechnicianPage
{
    ...
    public function handle(string $method, array $query, array $input): string
    {
        ...
        $message = null;
        if ($method === 'POST' && ($input['action'] ?? '') === 'save_room_default') {
            if (!CsrfToken::valid($input['csrf_token'] ?? null)) $message = 'The form expired. Please try again.';
            else { $this->store->saveDefaultRoomIds($this->organisationId, $this->userId, is_array($input['room_ids'] ?? null) ? $input['room_ids'] : []); $message = 'My rooms saved.'; }
        }
        ...
        $defaults = $this->store->defaultRoomIds($this->organisationId, $this->userId);
        ...
        $body = '<section class="page-header"><div><p class="eyebrow">Technician</p><h1>Daily preparation</h1></div><button type="button" onclick="window.print()">Print day</button></section>';
    ...
    {
        $html = '<section class="technician-room-controls"><form method="get"><input type="hidden" name="date" value="' . $this->e((string) ($_GET['date'] ?? 'today')) . '"><label>Rooms<select name="rooms" onchange="this.form.submit()"><option value="my"' . ($mode === 'my' ? ' selected' : '') . '>My rooms</option><option value="all"' . ($mode === 'all' ? ' selected' : '') . '>All rooms</option><option value="custom"' . ($mode === 'custom' ? ' selected' : '') . '>Custom room selection</option></select></label>';
        ...
        $html .= '<button class="secondary">Apply display</button></form><form method="post"><input type="hidden" name="csrf_token" value="' . $this->e(CsrfToken::value()) . '"><input type="hidden" name="action" value="save_room_default">';
        foreach ($rooms as $room) $html .= '<label class="check-label"><input type="checkbox" name="room_ids[]" value="' . (int) $room['id'] . '"' . (in_array((int) $room['id'], $defaults, true) ? ' checked' : '') . '> ' . $this->e($room['code']) . '</label>';
        return $html . '<button>Save My rooms</button></form></section>';
    }
    ...
    private function grid(DateTimeImmutable $date, array $rooms, array $selected, array $slots, array $occurrences): string
    {
        ...
        $html = '<section class="technician-sheet"><h2 class="print-date">Daily preparation · ' . $this->e($date->format('l j F Y')) . '</h2><div class="timetable-scroll"><table class="technician-grid"><thead><tr><th>Period</th>'; foreach ($rooms as $room) $html .= '<th>' . $this->e($room['code']) . '</th>'; $html .= '</tr></thead><tbody>';
        foreach ($slots as $slot) { $separator = ($slot['kind'] ?? '') !== 'teaching'; $html .= '<tr' . ($separator ? ' class="technician-separator"' : '') . '><th>' . $this->e((string) $slot['label']) . '</th>'; foreach ($rooms as $room) { $occurrence = $separator ? null : $this->occurrenceForSlot($byRoom[$room['code']] ?? [], $slot, $slots); $html .= '<td>' . ($separator ? '' : ($occurrence === null ? '<span class="muted">—</span>' : $this->cell($occurrence))) . '</td>'; } $html .= '</tr>'; }
    ...
    private function cell(array $occurrence): string
    {
        ...
        return '<details class="technician-cell"><summary title="' . $this->e($label) . '">' . $this->e($label) . '</summary><div class="technician-detail"><strong>' . $this->e((string) $occurrence['teacher_name']) . '</strong> · ' . $this->e((string) $occurrence['snapshot_class_code']) . ' · ' . $this->e((string) $occurrence['snapshot_room_code']) . '<br>' . $this->e((string) $occurrence['lesson_date']) . ' · ' . $this->e((string) $occurrence['period_label']) . '<p>' . nl2br($this->e($text === '' ? 'No requisition has been entered.' : $text)) . '</p><details><summary>Lesson details</summary><p>Lesson outline: ' . nl2br($this->e((string) ($occurrence['planning_notes'] ?? 'Not entered.'))) . '</p><p>Risk assessment: ' . nl2br($this->e((string) ($occurrence['risk_assessment_text'] ?? 'Not entered.'))) . '</p></details></div></details>';
    }
