...
final class AccountRecoveryPage
{
    ...
    private function verificationForm(?string $message): string
    {
        ...
        return '<section class="content-narrow"><p class="eyebrow">' . $this->e($this->organisation['name']) . '</p><h1>Account Recovery</h1>' . $notice . '<p>Recover an existing administrator account for this school. This cannot create an account or change roles.</p><div class="notice"><strong>The recovery key is highly sensitive.</strong> Anyone who has it may be able to take control of administrator accounts. Use it only on this secure school page.</div><form method="post"><input type="hidden" name="csrf_token" value="' . $this->e(CsrfToken::value()) . '"><input type="hidden" name="action" value="verify"><label>Administrator initials<input class="staff-identifier" name="staff_identifier" maxlength="3" pattern="[A-Za-z]{3}" autocomplete="username" required autofocus></label><label>Organisation Recovery Key<input type="password" name="recovery_key" maxlength="128" autocomplete="off" required></label><button>Continue account recovery</button></form><p><a href="/login">Return to login</a></p></section>';
    }

    private function passwordForm(?string $message): string
    {
        $notice = $message === null ? '' : '<p class="notice error">' . $this->e($message) . '</p>';
        return '<section class="content-narrow"><p class="eyebrow">' . $this->e($this->organisation['name']) . '</p><h1>Choose a new password</h1>' . $notice . '<p>The recovery details were verified. Choose a password to continue. Existing sessions for this administrator will be revoked.</p><form method="post"><input type="hidden" name="csrf_token" value="' . $this->e(CsrfToken::value()) . '"><input type="hidden" name="action" value="password"><label>New password<input type="password" name="password" minlength="8" maxlength="4096" autocomplete="new-password" required></label><label>Confirm new password<input type="password" name="password_confirmation" minlength="8" maxlength="4096" autocomplete="new-password" required></label><button>Reset password and rotate recovery key</button></form><form method="post"><input type="hidden" name="csrf_token" value="' . $this->e(CsrfToken::value()) . '"><input type="hidden" name="action" value="cancel"><button class="secondary">Cancel and start again</button></form></section>';
    }
