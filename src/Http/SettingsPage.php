...
final class SettingsPage
{
    ...
    public function handle(string $method, array $input): string
    {
        ...
        if ($method === 'POST') {
            try {
                if (array_key_exists('technician_highlighting_enabled', $input) || array_key_exists('technician_highlighting_count', $input) || array_key_exists('technician_highlighting_label', $input)) {
                    if (!CsrfToken::valid($input['csrf_token'] ?? null)) throw new SettingsValidationException(['The form expired. Please try again.']);
                }
    ...
    private function technicianHighlightingSection(array $data): string
    {
        $enabled = !empty($data['technician_highlighting_enabled']);
        $count = max(1, min(SettingsService::TECHNICIAN_HIGHLIGHTING_MAX, (int) ($data['technician_highlighting_count'] ?? count((array) ($data['technician_highlighting_colours'] ?? [])))));
        $labels = array_values((array) ($data['technician_highlighting_colours'] ?? []));
        $rows = '';
        for ($index = 0; $index < SettingsService::TECHNICIAN_HIGHLIGHTING_MAX; $index++) {
            $number = $index + 1;
            $rows .= '<div class="technician-highlight-setting" data-highlight-option><span class="technician-highlight-swatch technician-highlight-' . $number . '" aria-hidden="true"></span><label>Colour ' . $number . ' label<input name="technician_highlighting_label[]" maxlength="100" value="' . $this->e((string) ($labels[$index] ?? '')) . '" placeholder="For example: Waiting for equipment"></label></div>';
        }
        return '<div class="settings-section"><h2>Technician highlighting</h2><p>Optionally highlight whole technician lesson cells. Each school defines what its colours and labels mean; highlighting is independent of the prepared status.</p><label class="check-label"><input type="checkbox" name="technician_highlighting_enabled" value="1" data-highlight-enabled' . ($enabled ? ' checked' : '') . '> Enable technician highlighting</label><label>Number of available colours<select name="technician_highlighting_count" data-highlight-count>' . implode('', array_map(static fn (int $number): string => '<option value="' . $number . '"' . ($number === $count ? ' selected' : '') . '>' . $number . '</option>', range(1, SettingsService::TECHNICIAN_HIGHLIGHTING_MAX))) . '</select></label><div data-highlight-options>' . $rows . '</div></div>';
    }
    ...
    {
        return "function refreshSeparatorPositions(){var count=parseInt(document.querySelector('[data-period-count]').value,10)||1;document.querySelectorAll('[name=separator_after\\[\\]]').forEach(function(select){var selected=select.value;select.innerHTML='<option value=\"\">Select position</option>';for(var i=1;i<count;i++){var option=document.createElement('option');option.value=i;option.textContent='After period '+i;option.selected=String(i)===selected;select.appendChild(option);}});}function refreshHighlightOptions(){var enabled=document.querySelector('[data-highlight-enabled]');var count=parseInt(document.querySelector('[data-highlight-count]').value,10)||1;document.querySelectorAll('[data-highlight-option]').forEach(function(row,index){row.hidden=!enabled.checked||index>=count;});}function validateWorkingDays(){var days=document.querySelectorAll('[data-working-day]');if(!days.length)return;var valid=Array.from(days).some(function(day){return day.checked;});days[0].setCustomValidity(valid?'':'Select at least one working day.');}document.addEventListener('change',function(event){if(event.target.matches('[data-working-day]'))validateWorkingDays();if(event.target.matches('[data-highlight-enabled],[data-highlight-count]'))refreshHighlightOptions();});document.addEventListener('input',function(event){if(event.target.matches('[data-period-count]'))refreshSeparatorPositions();});document.addEventListener('DOMContentLoaded',function(){validateWorkingDays();refreshHighlightOptions();});document.addEventListener('click',function(event){var target=event.target;if(target.matches('[data-add-separator]')){var t=document.getElementById('separator-template');document.querySelector('[data-separators]').insertAdjacentHTML('beforeend',t.innerHTML);refreshSeparatorPositions();}if(target.matches('[data-remove]')){var parent=target.closest('[data-separators] > div');if(parent)parent.remove();}});";
    }
